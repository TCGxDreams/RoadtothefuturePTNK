  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 border-b ${scrolled
      ...
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        >
          <div className={`p-2 rounded-xl transition-all duration-300 ${scrolled || !isHome ? 'bg-amber-100/50' : 'bg-white/10 backdrop-blur-md'
            }`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {!isActive && (
                  <span className={`absolute inset-0 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full ${scrolled || !isHome ? 'bg-stone-100' : 'bg-white/10'
                    }`}></span>
                )}
        ...
        {/* Mobile Toggle */}
        <button
          className={`md:hidden p-3 rounded-xl transition-colors active:scale-95 ${scrolled || !isHome
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl shadow-2xl transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) overflow-hidden border-t border-stone-100 ${isOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
        }`}>
        <div className="flex flex-col p-6 space-y-3">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleNavClick(item.id)}
              className={`w-full text-left px-5 py-4 rounded-2xl font-medium transition-all duration-200 flex justify-between items-center group ${activePage === item.id
