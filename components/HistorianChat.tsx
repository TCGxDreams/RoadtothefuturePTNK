...
export const HistorianChat: React.FC<HistorianChatProps> = ({ currentPage }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] flex flex-col items-end pointer-events-none font-sans">
      {/* Chat Window */}
      <div className={`pointer-events-auto bg-white/95 backdrop-blur-2xl rounded-3xl shadow-[0_25px_60px_-10px_rgba(0,0,0,0.6)] w-[90vw] sm:w-[450px] mb-6 overflow-hidden transition-all duration-500 cubic-bezier(0.2, 0.8, 0.2, 1) origin-bottom-right border border-white/50 ring-1 ring-black/5 ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-90 opacity-0 translate-y-10 pointer-events-none'
        }`}>
        {/* Header */}
        ...
        <div className="h-[450px] overflow-y-auto p-5 bg-stone-50/80 space-y-5 custom-scrollbar">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-3 animate-fade-up ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center mt-1 shadow-md border ${msg.role === 'user'
                  ? 'bg-white border-stone-200 text-stone-600'
                  : 'bg-gradient-to-br from-amber-100 to-white text-amber-800 border-amber-200'
                }`}>
                {msg.role === 'user' ? <User size={16} /> : <Sparkles size={16} />}
              </div>
              <div className={`max-w-[85%] rounded-2xl px-5 py-3.5 text-sm leading-relaxed shadow-sm border ${msg.role === 'user'
